...
export const processTranscriptions = async () => {
  ...
  logMessages.push(`🔍 Leyendo archivos de transcripciones. Total encontrados: ${transcriptions.length}`);
  ...
  for (const transcription of transcriptions) {
    ...
    if (existingResource.length > 0) {
      logMessages.push(`⚠️ El video con ID ${id} ya está procesado. Saltando...`);
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ message: 'Transcripciones procesadas.', logs: [] });
  } catch (error) {
    return NextResponse.json({ message: 'Error al procesar transcripciones.', error: error }, { status: 500 });
  }
}
